...
class Patient(object):
    ...
    def update(self):
        ...
        while i < len(self.viruses):
            if True==  self.viruses[i].doesClear():
                del self.viruses[i]
            else:
                i=i+1
        
        population_density=float(len(self.viruses))/ float(self.maxPop)
        list=self.viruses[:]
        for i in list:
            try:
                self.viruses.append(i.reproduce(population_density))
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for time in range(300):
        sum = 0
        for i in range(numTrials):
            sum+= patients[i].update()
        average = float(sum)/float(numTrials)
        ylist.append(average)
        
    pylab.figure(1)
    pylab.plot(ylist, label = '# of Viruses')
    pylab.title('Number of Viruses for a Patient without Drug Treatment')
    pylab.xlabel('Time')
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in self.resistances:
            if (not self.isResistantTo(drug)) and drug in activeDrugs:
                raise NoChildException
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        index = 0
        while index < len(self.viruses):
            if self.viruses[index].doesClear():
